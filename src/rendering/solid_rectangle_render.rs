...
use crate::rendering::prelude::{Color};

const FLOATS_PER_VERTEX: i32 = 5;
    ...
    pub fn render(&self,
        ...
        unsafe {
            ...
            let vertices: [f32; BUFFER_SIZE] = [
                position.x, position.y,                   color[0], color[1], color[2],         // Top-left
                position.x + width, position.y,           color[0], color[1], color[2],         // Top-right
                position.x + width, position.y + height,  color[0], color[1], color[2],         // Bottom-right

                position.x, position.y + height,          color[0], color[1], color[2],         // Bottom-left
                position.x + width, position.y + height,  color[0], color[1], color[2],         // Bottom-right
                position.x, position.y,                   color[0], color[1], color[2],         // Top-left
            ];
